...
//         if(confirm("Xác nhận Đồng bộ sang NHLK?")) callApi("/system2/api/sync-bank", {});
//     });
...
//         await callApi("/system2/api/reset", {});
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function renderQueue(queue) {
        ...
        const container = document.getElementById("queueContainer");
        if (queue && queue.length > 0) queue.sort((a, b) => new Date(a.created_at) - new Date(b.created_at));
        const filteredQueue = (queue || []).filter(item => item.type !== 'ALLOCATION_CASH_PAID');
        ...
        const rowsHtml = filteredQueue.map(item => {
            ...
            if (item.type === 'LIQUIDATE_CD') { displayType = 'User bán CD'; displayClass = 'q-liq'; 
                if (details.sold) detailHtml = `<div class="mt-1 text-muted small fst-italic text-center">${details.sold.map(s => `<b>${s.soLuong}</b> x ${s.maCD}`).join(', ')}</div>`;
            } else if (item.type === 'ALLOCATION_ASSET_DELIVERED') { displayType = 'User Mua CD'; displayClass = 'q-alloc'; 
                const asset = details.assets?.[0];
                if (asset) detailHtml = `<div class="mt-1 text-muted small fst-italic text-center">${asset.soLuong} x ${asset.maCD}</div>`;
            } else if (item.type === 'CASH_IN') { displayType = 'Nạp Tiền'; displayClass = 'q-cash-in'; }
            else if (item.type === 'CASH_OUT') { displayType = 'Rút Tiền'; displayClass = 'q-cash-out'; }
            else return '';
    ...
    document.getElementById("btnSyncDiff")?.addEventListener("click", () => {
        ...
        const dateDisplay = selectedDate.split('-').reverse().join('/');
        if(confirm(`Xác nhận sync chênh lệch cho ngày: ${dateDisplay}?`)) {
            callApi("/system2/api/syncDiff", { date: selectedDate, user_id: TEST_USER_ID }, "Đồng bộ chênh lệch thành công");
        }
    });
